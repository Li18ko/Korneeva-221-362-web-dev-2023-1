...
function evaluate(str) {
    const OPNstr = compile(str); // Преобразовать выражение в ОПН
    const stack = [];
    const tokens = OPNstr.split(' ');
    ...
    for (const token of tokens) {
        if (!isNaN(token)) {
            ...
            // Если токен - оператор, выполняем операцию над верхними элементами стека
...
function clickHandler(event) {
    ...
    if (clickedButton.classList.contains('digit') || clickedButton.classList.contains('operation') || clickedButton.classList.contains('bracket')) {
        // Если нажата кнопка с классами digit, operation или bracket,
        // добавляем её текст к текущему содержимому экрана.
        output.textContent += buttonText;
    } else if (clickedButton.classList.contains('clear')) {
        // Если нажата кнопка с классом clear, очищаем экран.
        output.textContent = '';
    } else if (clickedButton.classList.contains('result')) {
        // Если нажата кнопка с классом result, вычисляем результат.
        const inputExpression = output.textContent;
        const result = evaluate(inputExpression);
        // Отображаем результат на экране с точностью до двух знаков после десятичного разделителя.
